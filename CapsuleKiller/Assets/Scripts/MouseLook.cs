...
public class MouseLook : MonoBehaviour {
	...
	void Update () {
        ...
        smoothVector.x = Mathf.Lerp(smoothVector.x, deltaMove.x, 1f / smoothing); // Use linear interpolation to move the camera smoothly on the x axis based on user input
        smoothVector.y = Mathf.Lerp(smoothVector.y, deltaMove.y, 1f / smoothing); // Use linear interpolation to move the camera smoothly on the y axis based on user input
